...
class Fasta:
    ...
    def __init__(self, f, g, gradf, proxg, beta=0.5, n_iter=1000, verbose=0):
        self.f = f
        self.g = g
        self.grad = gradf
        self.prox = proxg
    ...
    def learn(self, coefs_init, tol=1e-2):
        ...
        start = time.time()
        if self.verbose > 1:
            print("Iteration \t objective value \t step-size \t backtracking steps taken \t residual")
        for i in range(self.n_iter):
            if self.verbose > 1:
                if i % 25 == 0:
                    print(f"{current_process().name}: iterate {i}/{self.n_iter}")
            ...
            if self.verbose > 1:
                ...
                print(f"{i} \t {self.objective[i]} \t {self.stepsizes[i]} \t {self.backtracks[i]} \t {self.residuals[i]}")
