...
def nlgc_map_restingstate(name, evoked, forward, noise_cov, labels, order, n_frames, self_history=None, n_eigenmodes=2, alpha=0.0, beta=0.0,
        patch_idx=[], n_segments=1, loose=0.0, depth=0.0, pca=True, rank=None, lambda_range=None,
        max_iter=500, max_sweep_iter=5, max_cyclic_iter=3, tol=1e-5, sparsity_factor=0.0, cv=5, use_lapack=True, use_es=True,
        var_thr=1.0):
    """NLGC connectivity map estimation
    ...
    # d_raw_, bias_r_, bias_f_, models, conv_flag_ = \
    model_f =\
        _gc_extraction_restingstate(M, G, r, tt, n_frames, p=order, p1=self_history,
                        n_eigenmodes=n_eigenmodes,
                        ROIs=patch_idx, cv=cv, lambda_range=lambda_range, max_iter=max_iter, max_sweep_iter=max_sweep_iter,
                        max_cyclic_iter=max_cyclic_iter, tol=tol, sparsity_factor=sparsity_factor,
                        use_lapack=use_lapack, use_es=use_es, var_thr=var_thr)
    # d_raw[this_segment] = d_raw_
    # bias_r[this_segment] = bias_r_
...
def _gc_extraction_restingstate(y, f, r, tt, n_frames, p, p1, n_eigenmodes=2, var_thr=1.0, ROIs=[], 
        lambda_range=None, max_iter=500, max_sweep_iter=5, max_cyclic_iter=3,
        tol=1e-5, sparsity_factor=0.0, cv=5, use_lapack=True, use_es=True):
    n, m = f.shape
    nx = m // n_eigenmodes
    ...
    if len(lambda_range) > 1:
        raise Exception("resting state does not support crossvalidation")
        model_f = NeuraLVARCVRS(p, p1, n_eigenmodes, 10, cv, n_jobs, use_lapack=use_lapack)
    else:
        model_f = NeuraLVARRS(p, n_frames, p1, n_eigenmodes, use_lapack=use_lapack)
        lambda_range = lambda_range[0]

    model_f.fit(y, f, r * np.eye(n), [lambda_range]*n_frames, **kwargs)
