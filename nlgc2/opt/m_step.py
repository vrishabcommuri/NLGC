...
def solve_for_a_rs(q, s1, s2, a, a_prev, beta, sigma2, p1, lambda2, max_iter=5000, tol=1e-3, zeroed_index=None, update_only_target=False,
        n_eigenmodes=1):
    if not update_only_target or zeroed_index is None:
        if not a_prev is None: 
            return _solve_for_a_rs(q, s1, s2, a, a_prev, beta, sigma2, p1, lambda2, max_iter=max_iter, tol=tol, zeroed_index=zeroed_index,
                                n_eigenmodes=n_eigenmodes)
        else:
            return _solve_for_a(q, s1, s2, a, p1, lambda2, max_iter=max_iter, tol=tol, zeroed_index=zeroed_index,
                                n_eigenmodes=n_eigenmodes)
    else:
        raise NotImplementedError("update_only_target not implemented")
    
        

def _solve_for_a_rs(q, s1, s2, a, a_prev, beta, sigma2, p1, lambda2, max_iter=5000, tol=1e-3, zeroed_index=None, n_eigenmodes=1):        
    """Gradient descent to learn a, state-transition matrix
    ...
    s2 : ndarray of shape (n_sources*order, n_sources*order)
    a : ndarray of shape (n_sources, n_sources*order)
    a_prev : ndarray of shape (n_sources, n_sources*order)
    beta : int
    sigma2 :  int
    lambda2 : float
    max_iter : int, default=1000
    ...
    for i in range(max_iter):
        ...
        # remove normalization from the current a matrix
        acurr = a / d[None, :]
        acurr = acurr / q_inv_sqrt

        # add the continuity term to the gradient
        grad += (acurr - beta*a_prev)/sigma2
...
def solve_for_beta(a_s):
    num = 0
    for i in range(1, len(a_s)):
        num += (a_s[i] * a_s[i-1]).sum()
...
def solve_for_sigma2(a_s, beta):
    n = a_s[0].shape[0]
    nm = a_s[0].shape[1] 
    val = 0
    for i in range(1, len(a_s)):
        val += 1/(n*nm) * ((a_s[i] - beta*a_s[i-1])**2).sum()
    
    # add the first frame not included in loop above
    val += 1/(n*nm) * (a_s[i]**2).sum()
    return val
...
def compute_ll(y, x_, s, s_, s_hat, a, f, q, r, m, n, p):
    ...
    # val = T * sum_{i=1}^M log(1/sigma_i^2)
    val = t * np.log(c_inv[diag_indices]).sum()
    diff1 = diff1.dot(c_inv.T)

    # val = T * sum_{i=1}^M log(1/sigma_i^2) - sum_{i=1}^M 1/(2sigma_i^2) ||x-Xai||_2
    val -= (diff1 * diff1).sum() / 2
    ...
    # val = T * log(|R|); see https://blogs.sas.com/content/iml/2012/10/31/compute-the-log-determinant-of-a-matrix.html
    val += t * np.log(c_inv[diag_indices]).sum()
    diff2 = diff2.dot(c_inv.T)

    # val = T * log(|R|) - sum_{t=1}^T 1/(2) ||y_t-Cxt||_{R^{-1}}
    val -= (diff2 * diff2).sum() / 2

    ## A.3 continued (expanding G^(l))
    # I think this term comes from the fact that E[X^TX|y_{1:T}] = m_tm_t^T + \hat S 
    # the above calculations don't include the extra smoother covariance (\hat S) term
    # and we therefore add it back here. I think that we drop some coefficients from out front
    # since it just scales the LL and it makes the code simpler to exclude them.


    diag_indices = np.diag_indices(m)
    # c = linalg.cholesky(s_hat[(p - 1) * m:, (p - 1) * m:], lower=True)

    # I think we break this into two steps due to convergence errors hence the try except block.
    # really this should ideally work for c = linalg.cholesky(s_hat, lower=True), i.e., all indices
    try:
        c = linalg.cholesky(s_hat[(p - 1) * m:, (p - 1) * m:], lower=True)
...
def compute_ll_rs(y, x_, s, s_, s_hat, a, a_s, beta, sigma2, f, q, r, m, n, p):
    """Computes log-likelihood (See README)
    ...
    # resting state continuity between frames 
    val -= np.log(sigma2)
    for i in range(1, len(a_s)):
        val -= 1/sigma2 * ((a_s[i] - beta*a_s[i-1])**2).sum()

    # subtract off the first frame not included in loop above
    val -= 1/sigma2 * ((a_s[0])**2).sum()
    return val
