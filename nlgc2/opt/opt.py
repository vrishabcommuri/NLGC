...
from .m_step import (calculate_ss, solve_for_a, solve_for_a_rs, solve_for_q, compute_ll, compute_ll_rs, 
                     compute_cross_ll, compute_Q, solve_for_beta, solve_for_sigma2)
...
class NeuraLVARRS(NeuraLVAR):
    def __init__(self, order, n_frames, self_history=None, n_eigenmodes=None, copy=True, 
                 standardize=False, normalize=False, use_lapack=True):
        super().__init__(order, self_history, n_eigenmodes, copy, standardize, normalize, use_lapack)
        self.n_frames = n_frames

    def fit(self, y, f, r, lambda2=None, max_iter=500, max_cyclic_iter=3, max_sweep_iter=5, rel_tol=0.0001,
            restriction=None, use_es=None):
        """Fits the model from given m/eeg data, forward gain and noise covariance
        ...
        if y.shape[1] / self.n_frames != y.shape[1] // self.n_frames:
            raise ValueError(f"number of frames {self.n_frames} must evenly divide data time series of length {y.shape[1]}")

        a, q_upper, lls, f, r, zeroed_index, _, x_ = self._fit(y, f, r, lambda2=lambda2, max_iter=max_iter, max_sweep_iter=max_sweep_iter,
                                                               max_cyclic_iter=max_cyclic_iter, rel_tol=rel_tol)
        self._parameters = (a, f, q_upper, r, x_)
        self._zeroed_index = zeroed_index
        self._lls = lls
        self.ll = lls[0][-1]
    ...
    def _fit(self, y, f, r, lambda2=None, max_iter=500, max_cyclic_iter=3, max_sweep_iter=5,
            rel_tol=0.01, xs=None, fixed_a=False, fixed_q=False):
        """Internal function that fits the model from given data
        ...
        if self.restriction is not None:
            raise NotImplementedError(f"restriction {self.restriction} != None is not supported for resting state")
        else:
            zeroed_index = None

        totaltime = y.shape[1]
        seg_len = totaltime // self.n_frames

        
        a_s = [None]*self.n_frames
        _, m = f.shape
        q_s = [0.0001 * np.eye(m)]*self.n_frames

        if lambda2 is None:
            lambda2 = [0] * self.n_frames
        

        beta = 0.5
        sigma2 = 1

        x_s = [None]*self.n_frames

        final_model_infos = []
        for j in range(max_sweep_iter):
            print("sweep ", j)
            # one model for each frame
            model_infos = []
            for curr_seg, start in enumerate(range(0, totaltime, seg_len)):
                lls = []
                ll_s = []
                Qvals = []
                source_fits = []

                # only look at the observations for the current chunk
                end = start + seg_len
                print("segment ", curr_seg, "start ", start, "end ", end)
                y_frame = y[:, start:end]
                
                # use the current best guess at A and Q for this frame as the initialization (both start as None which is the zero matrix)
                y_frame, a_, a_upper, f_, q_, q_upper, non_zero_indices, r, xs, m, n, p, use_lapack = \
                    self._prep_for_sskf(y_frame,  a_s[curr_seg], f, q_s[curr_seg], r, xs)
                ...
                for i in range(max_iter):
                    ...
                    x_, s, s_, b, s_hat, ll_ = sskf(y_frame, a_, f_, q_, r, xs=xs, use_lapack=use_lapack)
                    x_s[curr_seg] = x_
                    ll_s.append(ll_)
                    # still on first iteration, haven't populated a matrices yet
                    if a_s[-1] is None: 
                        ll = compute_ll_rs(y_frame, x_, s, s_, s_hat, a_upper, [np.zeros_like(a_upper)]*self.n_frames, beta, sigma2, f, q_upper, r, m, n, p)
                    else:
                        ll = compute_ll_rs(y_frame, x_, s, s_, s_hat, a_upper, [self._ravel_a(a) for a in a_s], beta, sigma2, f, q_upper, r, m, n, p)
                    lls.append(ll)
                    Qvals.append(compute_Q(y_frame, x_, s_, b, a_upper, f, q_upper, r, m, p))
                    source_fits.append(compute_cross_ll(x_, a_upper, q_upper, m, p))
                    # stopping cond
                    if i > 0:
                        rel_change = (lls[i - 1] - lls[i]) / lls[i - 1]
                    ...
                    for _ in range(max_cyclic_iter):
                        if not fixed_a:
                            if curr_seg == 0:
                                ...
                                a_prev = self._ravel_a(a_s[curr_seg - 1])
                                if a_prev is None:
                                    a_prev = np.zeros_like(a_upper)
                            a_upper, changes = solve_for_a_rs(q_upper, s1, s2, a_upper, a_prev, beta, sigma2, p1, lambda2=lambda2[curr_seg], max_iter=5000,
                                                        tol=min(1e-4, rel_tol), zeroed_index=zeroed_index,
                                                        update_only_target=False, n_eigenmodes=self._n_eigenmodes)
                        if not fixed_q:
                            q_upper, rel_q_change = solve_for_q(q_upper, s3, s1, s2, a_upper, lambda2=lambda2[curr_seg])
                    ...
                    a_s[curr_seg] = self._unravel_a(a_upper)
                    q_s[curr_seg] = q_upper
                    
                    
                model_infos.append((lls, ll_s, Qvals, source_fits))
                
            beta = solve_for_beta(a_s)
            sigma2 = solve_for_sigma2(a_s, beta)
            final_model_infos.append(model_infos + [beta, sigma2])

        

        return a_s, q_s, final_model_infos, f, r, zeroed_index, xs, x_s
